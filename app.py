...
for k, v in {
    ...
    'results':None,'api_key':'','engine':'Ensemble (Prophet + ETS)',
    'history_months':0,'ai_insights':None
}.items():
...
def run_forecast(df, zone_cols, horizon):
    ...
    for zone in zone_cols:
        ...
        try:
            ...
            pb=mb.predict(fb).tail(test_size)['yhat'].values
            prophet_backtest=pb
        except:
            prophet_fc=None
        ...
        try:
            ...
            ets_fit=ets_model.fit(disp=False,maxiter=500)
            ets_fc=ets_fit.forecast(horizon)

            ets_b=ETSModel(train['y'],error='mul',trend='add',seasonal='mul',
                           seasonal_periods=12,initialization_method='estimated')
            ets_bf=ets_b.fit(disp=False,maxiter=500)
            ets_backtest=ets_bf.forecast(test_size)
        except:
            try:
                from statsmodels.tsa.holtwinters import ExponentialSmoothing
                hw=ExponentialSmoothing(series['y'],trend='add',seasonal='add',seasonal_periods=12)
                hw_fit=hw.fit(optimized=True)
                ets_fc=hw_fit.forecast(horizon)
                hw_b=ExponentialSmoothing(train['y'],trend='add',seasonal='add',seasonal_periods=12)
                hw_bf=hw_b.fit(optimized=True)
                ets_backtest=hw_bf.forecast(test_size)
            except:
                ets_fc=None
        ...
        if prophet_fc is not None and ets_fc is not None:
            ...
            if prophet_backtest is not None and ets_backtest is not None:
                backtest_fc=np.array(prophet_backtest)*0.70+np.array(ets_backtest)*0.30
            elif prophet_backtest is not None:
                backtest_fc=prophet_backtest
            else:
                backtest_fc=np.array(ets_backtest)
        elif prophet_fc is not None:
            base_fc=np.array(prophet_fc)
            backtest_fc=prophet_backtest if prophet_backtest is not None else np.array(hist_vals[-test_size:])
        elif ets_fc is not None:
            base_fc=np.array(ets_fc)
            backtest_fc=np.array(ets_backtest) if ets_backtest is not None else np.array(hist_vals[-test_size:])
        else:
        ...
        # Smoothing — cap spikes vs historical mean
        hist_mean=np.mean(hist_vals)
        hist_std=np.std(hist_vals)
        cap=hist_mean+3*hist_std
        floor=max(hist_mean-3*hist_std,0)
        base_fc=np.clip(base_fc,floor,cap)
...
def get_ai_insights(api_key,utility,unit,horizon,results,zone_cols):
    try:
        ...
        for z in zone_cols[:8]:
            r=results[z]
            lines.append(f"- {z}: avg {round(np.mean(r['vals_hist'][-12:]),1)} {unit}, forecast end {r['base'][-1]} {unit}, trend {r['trend_pct']:+.1f}%, MAPE {r['mape']}%")
...
2. Key zones to watch — highest growth and anomalies (3-4 bullets)
3. Strategic infrastructure recommendations (3-4 bullets)
4. Model confidence note (1-2 sentences)

Use professional utility planning language. Be concise and precise."""
...
def build_excel(df,results,zone_cols,utility,unit,horizon):
    ...
    with pd.ExcelWriter(buf,engine='openpyxl') as writer:
        ...
        for z in zone_cols:
            r=results[z]
            dates_test=[d.strftime('%b %Y') if hasattr(d,'strftime') else str(d) for d in r['dates_test']]
            for i,(dt,act,pred) in enumerate(zip(dates_test,r['vals_test'],r['backtest_fc'])):
...
def build_pdf(results,zone_cols,utility,unit,horizon,ai_insights=None):
    try:
        ...
        title_style=ParagraphStyle('Title',parent=styles['Title'],
            fontSize=22,textColor=colors.HexColor('#4a2a8a'),
            spaceAfter=6,alignment=TA_CENTER,fontName='Helvetica-Bold')
        sub_style=ParagraphStyle('Sub',parent=styles['Normal'],
            fontSize=11,textColor=colors.HexColor('#6050a0'),
            spaceAfter=4,alignment=TA_CENTER)
        h2_style=ParagraphStyle('H2',parent=styles['Heading2'],
            fontSize=13,textColor=colors.HexColor('#4a2a8a'),
            spaceBefore=14,spaceAfter=6,fontName='Helvetica-Bold')
        body_style=ParagraphStyle('Body',parent=styles['Normal'],
            fontSize=10,textColor=colors.HexColor('#2a1a4a'),
            spaceAfter=4,leading=16)
        footer_style=ParagraphStyle('Footer',parent=styles['Normal'],
            fontSize=8,textColor=colors.HexColor('#8060b0'),alignment=TA_CENTER)

        story=[]

        # Cover
        story.append(Spacer(1,1*cm))
        story.append(Paragraph('🔮 Utility Crystal Ball',title_style))
        story.append(Paragraph('Consumption Forecast Report',sub_style))
        story.append(Paragraph(f'{utility} | Unit: {unit} | Horizon: {horizon} months | Zones: {len(zone_cols)}',sub_style))
        story.append(HRFlowable(width='100%',thickness=1,color=colors.HexColor('#8060d0'),spaceAfter=12))

        # Accuracy summary table
        story.append(Paragraph('Forecast Accuracy Summary',h2_style))
        tdata=[['Zone',f'Avg Historical ({unit})',f'Forecast End ({unit})','Growth (%)','MAPE Accuracy']]
        for z in zone_cols:
            r=results[z]
            mape_str=f"{r['mape']}%" if r['mape'] else 'N/A'
            tdata.append([z,
                f"{round(np.mean(r['vals_hist']),1):,.1f}",
                f"{r['base'][-1]:,.1f}",
                f"{r['trend_pct']:+.1f}%",
                mape_str])
        t=Table(tdata,colWidths=[3.5*cm,4*cm,4*cm,3*cm,3.5*cm])
        t.setStyle(TableStyle([
            ('BACKGROUND',(0,0),(-1,0),colors.HexColor('#4a2a8a')),
            ('TEXTCOLOR',(0,0),(-1,0),colors.white),
            ('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),
            ('FONTSIZE',(0,0),(-1,-1),9),
            ('ALIGN',(0,0),(-1,-1),'CENTER'),
            ('ROWBACKGROUNDS',(0,1),(-1,-1),[colors.HexColor('#f5f0ff'),colors.white]),
            ('GRID',(0,0),(-1,-1),0.5,colors.HexColor('#c0a0ff')),
        ...
        # Backtest validation table
        story.append(Paragraph('Backtest Validation — Actual vs Predicted (10% holdout)',h2_style))
        for z in zone_cols:
            r=results[z]
            story.append(Paragraph(f'Zone: {z}',ParagraphStyle('ZH',parent=body_style,fontName='Helvetica-Bold')))
            dates_test=[d.strftime('%b %Y') if hasattr(d,'strftime') else str(d) for d in r['dates_test']]
            bt_data=[['Month',f'Actual ({unit})',f'Predicted ({unit})','Error (%)']]
            for dt,act,pred in zip(dates_test,r['vals_test'],r['backtest_fc']):
                err=round(abs(act-pred)/max(act,1)*100,1) if act>0 else 0
                bt_data.append([dt,f'{act:,.1f}',f'{pred:,.1f}',f'{err:.1f}%'])
            bt=Table(bt_data,colWidths=[3.5*cm,4.5*cm,4.5*cm,3.5*cm])
            bt.setStyle(TableStyle([
                ('BACKGROUND',(0,0),(-1,0),colors.HexColor('#6a3aaa')),
                ('TEXTCOLOR',(0,0),(-1,0),colors.white),
                ('FONTNAME',(0,0),(-1,0),'Helvetica-Bold'),
                ('FONTSIZE',(0,0),(-1,-1),8),
                ('ALIGN',(0,0),(-1,-1),'CENTER'),
        ...
        # Forecast tables per zone
        story.append(Paragraph('Detailed Forecast — All Scenarios',h2_style))
        for z in zone_cols:
            r=results[z]
            story.append(Paragraph(f'Zone: {z} | Growth: {r["trend_pct"]:+.1f}% | MAPE: {r["mape"]}%',
                ParagraphStyle('ZH2',parent=body_style,fontName='Helvetica-Bold')))
            dates_fc=[d.strftime('%b %Y') if hasattr(d,'strftime') else str(d) for d in r['dates_fc']]
            fc_data=[['Month',f'Base ({unit})',f'Optimistic ({unit})',f'Conservative ({unit})']]
            for dt,b,o,c in zip(dates_fc,r['base'],r['optimistic'],r['conservative']):
                fc_data.append([dt,f'{b:,.1f}',f'{o:,.1f}',f'{c:,.1f}'])
            ft=Table(fc_data,colWidths=[3.5*cm,4.5*cm,4.5*cm,4.5*cm])
        ...
        if ai_insights:
            ...
            story.append(Paragraph('AI-Powered Executive Insights',h2_style))
        ...
        # Methodology
        story.append(HRFlowable(width='100%',thickness=1,color=colors.HexColor('#8060d0'),spaceAfter=8))
        story.append(Paragraph('Methodology Note',h2_style))
        methodology=f"""This report was generated using an ensemble forecasting approach combining Prophet (70% weight) 
and ETS/Exponential Smoothing (30% weight). Prophet captures yearly seasonality, trend, and calendar effects. 
ETS provides stable exponential smoothing to reduce volatility. The ensemble reduces individual model bias.

Backtest validation uses a 10% holdout period ({results[zone_cols[0]]['test_size']} months) to measure real predictive accuracy (MAPE).
Forecast horizon: {horizon} months. Three scenarios presented: Base (most likely), Optimistic (+6%), Conservative (-6%).

Developed by Engr. Ahmed Raslan | Engr_Raslan@outlook.com | +971 52 289 9595"""
        story.append(Paragraph(methodology,body_style))

        # Footer
        story.append(Spacer(1,0.5*cm))
        story.append(HRFlowable(width='100%',thickness=0.5,color=colors.HexColor('#c0a0ff')))
        story.append(Spacer(1,0.2*cm))
        story.append(Paragraph('🔮 Utility Crystal Ball | Developed by Engr. Ahmed Raslan | Engr_Raslan@outlook.com | +971 52 289 9595',footer_style))
...
def build_template(utility,unit):
    ...
    with pd.ExcelWriter(buf,engine='openpyxl') as writer:
        ...
        pd.DataFrame({'Instructions':[
            f'Utility: {utility}',f'Unit: {unit}',
            'Fill consumption values for each zone column.',
            'Add more zones as needed (Zone_4, Zone_5, etc.)',
            'Date format: YYYY-MM-DD','Minimum 60 months required.',
            '120+ months recommended for maximum accuracy.',
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with c:
        if st.button("⚡ Load example"):
            ...
            for i,(base,amp,trend) in enumerate(zip([1450,1880,1980],[400,80,620],[8,-3,15])):
                vals=[]
                for j in range(120):
                    vals.append(round(max(base+np.sin((j-3)*np.pi/6)*amp+trend*j/12+np.random.normal(0,base*.02),100),1))
                ex[f'Zone_{i+1}']=vals
            st.session_state.df=ex
            st.session_state.zones=['Zone_1','Zone_2','Zone_3']
            st.session_state.history_months=120
    ...
    for i,(util,info) in enumerate(UTILITIES.items()):
        with cols[i]:
            ...
            border='2px solid #8060d0' if sel else '1px solid #2a1a4a'
            bg='#241848' if sel else '#16102a'
            st.markdown(f'<div style="background:{bg};border:{border};border-radius:10px;padding:1rem .5rem;text-align:center;margin-bottom:.5rem"><div style="font-size:1.8rem">{info["icon"]}</div><div style="font-size:.75rem;color:{"#c0a0ff" if sel else "#8070a0"};margin-top:4px">{util}</div><div style="font-size:.65rem;color:#5a4a7a">{info["unit"]}</div></div>',unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("🤖 Optional: Anthropic API key for AI-powered insights"):
        st.markdown('<div class="info-box">Your key is used this session only — never stored. Get yours at console.anthropic.com</div>',unsafe_allow_html=True)
        api_key=st.text_input("Anthropic API key",type="password",value=st.session_state.api_key,placeholder="sk-ant-...")
    ...
    st.markdown(f'<div class="info-box">Template: <strong>Date</strong> column (YYYY-MM-DD) + zone columns (Zone_1, Zone_2...). Values in <strong>{unit}</strong>. Minimum 60 months. 120+ months recommended.</div>',unsafe_allow_html=True)
    ...
    if uploaded:
        try:
            ...
            if not valid:
                st.markdown('<div class="error-box">❌ Data did not pass validation. Please fix the issues above and re-upload.</div>',unsafe_allow_html=True)
            elif months<120:
                st.markdown(f'<div class="warn-box">⚠️ Your data contains {months} months. 120+ months recommended for maximum accuracy. You may proceed but results may have reduced confidence.</div>',unsafe_allow_html=True)
                st.session_state.df=df_raw;st.session_state.zones=zone_cols;st.session_state.history_months=months
            else:
                st.markdown(f'<div class="success-box">✅ Data validated — {months} months, {len(zone_cols)} zones. Ready to forecast!</div>',unsafe_allow_html=True)
                st.session_state.df=df_raw;st.session_state.zones=zone_cols;st.session_state.history_months=months
        except Exception as e:
    ...
    with c2:
        if st.session_state.df is not None and st.session_state.history_months>=60:
            ...
            st.info("Upload valid data (min 60 months) to run forecast")
    dev_footer()
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    steps_ui=["Validating data","Detecting seasonality patterns","Running Prophet model",
              "Running ETS model","Combining ensemble (70% Prophet + 30% ETS)",
              "Running walk-forward backtest validation","Generating 3 scenarios","Building reports"]
    pb=st.progress(0)
    sb=st.empty()
    ...
    for i,sn in enumerate(steps_ui):
        ...
        if i==2:
            results=run_forecast(df,zone_cols,horizon)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with c4:
        if avg_mape:
            ...
            st.markdown(f'<div class="metric-box"><div class="metric-label">Avg MAPE Accuracy</div><div class="metric-val">N/A</div></div>',unsafe_allow_html=True)

    st.markdown('---')
    tab1,tab2,tab3,tab4,tab5=st.tabs(["📈 Forecast Charts","🔍 Backtest Validation","🏆 Area Rankings","🤖 AI Insights","📥 Export"])

    # ── Tab 1: Forecast ──
    with tab1:
        zone_options=['All zones']+zones
        sel_zone=st.selectbox("Select zone",zone_options,key="zone_sel")
        def make_forecast_chart(zk):
            fig=go.Figure()
            if zk=='All zones':
                ...
                r=results[zk]
                hd,hv=r['dates_hist'],r['vals_hist']
                fd,bfc,ofc,cfc=r['dates_fc'],r['base'],r['optimistic'],r['conservative']
                title=f"{zk} — {util} ({unit})"
            fig.add_trace(go.Scatter(x=list(fd)+list(fd)[::-1],y=ofc+cfc[::-1],
                fill='toself',fillcolor='rgba(61,176,122,0.08)',
                line=dict(color='rgba(0,0,0,0)'),showlegend=False,hoverinfo='skip'))
            fig.add_trace(go.Scatter(x=hd,y=hv,name='Historical',
                line=dict(color='#8060d0',width=2),
                hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Historical</extra>'))
            fig.add_trace(go.Scatter(x=fd,y=bfc,name='Base forecast',
                line=dict(color='#3db07a',width=2),
                hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Base</extra>'))
            fig.add_trace(go.Scatter(x=fd,y=ofc,name='Optimistic',
                line=dict(color='#cc7040',width=1.5,dash='dash'),
                hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Optimistic</extra>'))
            fig.add_trace(go.Scatter(x=fd,y=cfc,name='Conservative',
                line=dict(color='#6a5a8a',width=1.5,dash='dash'),
                hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Conservative</extra>'))
            if fd:
                sx=str(fd[0])[:10]
                fig.add_shape(type='line',x0=sx,x1=sx,y0=0,y1=1,xref='x',yref='paper',
                    line=dict(color='#3a2a5a',width=1,dash='dot'))
                fig.add_annotation(x=sx,y=0.97,xref='x',yref='paper',
                    text='forecast →',showarrow=False,
                    font=dict(color='#5a4a7a',size=10),xanchor='left')
            fig.update_layout(
                ...
                xaxis=dict(title='Month',title_font=dict(color='#7060a0',size=11),
                    tickfont=dict(color='#7060a0',size=10),
                    gridcolor='#1e1438',tickformat='%b %Y',dtick='M6'),
                yaxis=dict(title=f'Consumption ({unit})',title_font=dict(color='#7060a0',size=11),
                    tickfont=dict(color='#7060a0',size=10),
                    gridcolor='#1e1438',tickformat=',.0f'),
                legend=dict(bgcolor='#16102a',bordercolor='#2a1a4a',borderwidth=1,
                    font=dict(color='#9080b0',size=10)),
                margin=dict(l=60,r=20,t=50,b=60),height=420,hovermode='x unified')
            return fig
        st.plotly_chart(make_forecast_chart(sel_zone),use_container_width=True)

    # ── Tab 2: Backtest ──
    with tab2:
        st.markdown('<div class="info-box">Walk-forward validation: last 10% of your data was held out for testing. The model was trained on the remaining 90% and predicted the holdout period. This shows real predictive accuracy.</div>',unsafe_allow_html=True)
        sel_bt=st.selectbox("Select zone for backtest",zones,key="bt_sel")
        r=results[sel_bt]
        fig_bt=go.Figure()
        fig_bt.add_trace(go.Scatter(x=r['dates_hist'],y=r['vals_hist'],name='Full historical',
            line=dict(color='#8060d0',width=2),opacity=0.5,
            hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Historical</extra>'))
        fig_bt.add_trace(go.Scatter(x=r['dates_test'],y=r['vals_test'],name='Actual (holdout)',
            line=dict(color='#3db07a',width=2.5),
            hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Actual</extra>'))
        fig_bt.add_trace(go.Scatter(x=r['dates_test'],y=r['backtest_fc'],name='Model predicted',
            line=dict(color='#ff8040',width=2.5,dash='dash'),
            hovertemplate='%{x|%b %Y}<br>%{y:,.1f} '+unit+'<extra>Predicted</extra>'))
        if r['dates_test']:
            sx=str(r['dates_test'][0])[:10]
            fig_bt.add_shape(type='line',x0=sx,x1=sx,y0=0,y1=1,xref='x',yref='paper',
                line=dict(color='#3a2a5a',width=1,dash='dot'))
            fig_bt.add_annotation(x=sx,y=0.97,xref='x',yref='paper',
                text='← training | test →',showarrow=False,
                font=dict(color='#5a4a7a',size=10),xanchor='center')
        fig_bt.update_layout(
            title=dict(text=f'{sel_bt} — Actual vs Predicted (Backtest) | MAPE Accuracy: {r["mape"]}%',
                font=dict(color='#c0a0ff',size=13)),
            paper_bgcolor='#12102a',plot_bgcolor='#12102a',
            font=dict(color='#9080b0',size=11),
            xaxis=dict(title='Month',title_font=dict(color='#7060a0',size=11),
                tickfont=dict(color='#7060a0',size=10),
                gridcolor='#1e1438',tickformat='%b %Y',dtick='M3'),
            yaxis=dict(title=f'Consumption ({unit})',title_font=dict(color='#7060a0',size=11),
                tickfont=dict(color='#7060a0',size=10),
                gridcolor='#1e1438',tickformat=',.0f'),
            legend=dict(bgcolor='#16102a',bordercolor='#2a1a4a',borderwidth=1,
        ...
        for dt,act,pred in zip(dates_t,r['vals_test'],r['backtest_fc']):
            err=round(abs(act-pred)/max(act,1)*100,1) if act>0 else 0
            bt_rows.append({'Month':dt,f'Actual ({unit})':act,f'Predicted ({unit})':pred,'Error (%)':f'{err}%'})
    ...
    with tab3:
        ...
        for z in zones:
            r=results[z]
            rows.append({'Zone':z,
                f'Avg Last 12mo ({unit})':round(np.mean(r['vals_hist'][-12:]),1),
                ...
                'Status':'🚀 High growth' if r['trend_pct']>5 else
                         ...
                         '⚠️ Anomaly' if r['trend_pct']<-1 else '➡️ Stable'})
        st.dataframe(pd.DataFrame(rows).sort_values('Growth (%)',ascending=False),
        ...
        with c2:
            ...
            if pdf_buf:
                st.download_button("⬇ Download PDF",data=pdf_buf,
                    file_name=f"UCB_{util.replace(' ','_')}_{horizon}mo_report.pdf",
